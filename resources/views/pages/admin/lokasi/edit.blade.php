...
<div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.lokasi.update', $lokasi->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama_lokasi" id="nama_lokasi" value="{{ old('nama_lokasi', $lokasi->nama_lokasi) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="aktif" id="aktif" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border" required>
                <option value="Y" {{ old('aktif', $lokasi->aktif) == 'Y' ? 'selected' : '' }}>Y - Aktif</option>
                <option value="N" {{ old('aktif', $lokasi->aktif) == 'N' ? 'selected' : '' }}>N - Tidak Aktif</option>
            </select>
        </div>

        <div class="flex justify-end gap-2">
            <a href="{{ route('admin.lokasi.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Batal</a>
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Perbarui Lokasi</button>
        </div>
    </form>
</div>
@endsection
